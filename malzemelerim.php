                        ...
                        foreach ($malzemeler as $malzeme) {
                            foreach ($malzemeler2 as $malzeme2) {
                                if ($malzeme == $malzeme2["malzeme_adi"]) {
                                    echo '
                                        <li><img style="margin: 1px;" src="yuklenendosyalar/malzemeler/' . $malzeme2['malzeme_gorsel'] . '" height="25" class="tarifMalzemeGorsel">
                                        ' . $malzeme2['malzeme_adi'] . '
                                        </li>';
                                }
                            }
                        }
                        ...
                        while ($row2 = $selectResult2->fetch_assoc()) {
                            foreach ($malzemeler as $malzeme) {
                                if ($row2["malzeme_adi"] == $malzeme) {
                                    echo '<li><img style="margin: 1px;" src="yuklenendosyalar/malzemeler/' . $row2['malzeme_gorsel'] . '" height="25" class="tarifMalzemeGorsel">
                                    <span style="color: rgb(37, 214, 37);">' . $row2["malzeme_adi"] . ' (' . $row2["malzeme_id"] . ')</span></li>';
                                }
                            ...
                            if (!in_array($m1, $malzemeler)) {
                                foreach ($malz as $row5) {
                                    if ($m1 == $row5["malzeme_adi"]) {
                                        echo '<li><img style="margin: 1px;" src="yuklenendosyalar/malzemeler/' . $row5['malzeme_gorsel'] . '" height="25" class="tarifMalzemeGorsel">
                                            <span style="color: red;">' . $row5["malzeme_adi"] . ' (' . $row5["malzeme_id"] . ')</span></li>';
                                    }
                                }
                            }
...
if (isset($_POST['ekle'])) {
    ...
    while ($rowMalzeme = $resultMalzeme->fetch_assoc()) {
        if ($eklenecek == $rowMalzeme["malzeme_id"]) {
            $eklenecekMalzAdi = $rowMalzeme["malzeme_adi"];
            while ($rowKullanici = $resultKullanici->fetch_assoc()) {
