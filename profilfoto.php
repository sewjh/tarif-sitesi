...
<body>
    <center>
        ...
        <div class="main-box" style="position: relative" ;>
            <center>
                <div class="lineBox">
                    <h1 class="baslik" style="font-size: 34px;">Profil Fotoğrafı</h1>
                </div>
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    <input type="file" name="pp" style="border-radius: 7px; background-color: white; color: black;"
                        required>>
                    <input type="submit" name="degistir" value="DEGISTIR" class="submitBtn">
                </form>
            </center>
        </div>
    </center>
...
if (isset($_POST['degistir'])) {
    ...
    while ($row = $result->fetch()) {
        if ($_SESSION["user"] == $row["kullanici_adi"]) {
            $update = "UPDATE kullanici set kullanici_foto = '" . $pp . "' where kullanici_id = " . $row['kullanici_id'];
            $upResult = $conn->exec($update);
            ...
            $dizin = 'profilfoto/';
            $yuklenecek_dosya = $dizin . basename($_FILES['pp']['name']);

            if (move_uploaded_file($_FILES['pp']['tmp_name'], $yuklenecek_dosya)) {
                echo "
                <script> alert('Profil fotoğrafı başarıyla değiştirildi!');
                </script>";
